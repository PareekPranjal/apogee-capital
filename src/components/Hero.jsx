    ...
    >
      ...
      <motion.div className="relative max-w-7xl mx-auto px-6 lg:px-8 text-center" style={{ opacity, scale }}>
        ...
        <motion.div
          ...
          transition={{ duration: 1, delay: d(0.1), ease: [0.22, 1, 0.36, 1] }}
        ...
        <div className="overflow-hidden mb-3">
          <motion.h1
            ...
            initial={{ y: "100%" }}
            animate={show ? { y: "0%" } : {}}
            transition={{ duration: 1.2, delay: d(0.2), ease: [0.22, 1, 0.36, 1] }}
          >
            Strategic Vision.
          </motion.h1>
        </div>
        <div className="overflow-hidden mb-3">
          <motion.h1
            ...
            initial={{ y: "100%" }}
            animate={show ? { y: "0%" } : {}}
            transition={{ duration: 1.2, delay: d(0.35), ease: [0.22, 1, 0.36, 1] }}
          >
            Trusted Execution.
          </motion.h1>
        </div>
        ...
        >
          <Magnetic strength={0.15}>
            <button onClick={() => scrollTo("#services")} className="btn-primary" data-cursor>
              <span>Explore Services</span> <ArrowRight size={16} />
            </button>
          </Magnetic>
          <Magnetic strength={0.15}>
            <button onClick={() => scrollTo("#about")} className="btn-outline" data-cursor>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-0">
            {[
              ...
              <motion.div
                ...
                initial={{ opacity: 0, y: 20 }}
                animate={show ? { opacity: 1, y: 0 } : {}}
                transition={{ duration: 0.6, delay: d(1.3 + i * 0.1) }}
                className="text-center relative"
              >
                {i > 0 && <div className="hidden md:block absolute left-0 top-1/2 -translate-y-1/2 w-px h-8 bg-white/[0.06]" />}
                <div className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-2 tabular-nums">
                  {stat.display || (show && <Counter value={stat.value} suffix={stat.suffix} delay={1.3 + i * 0.15} />)}
                </div>
                <div className="text-[10px] sm:text-xs tracking-[0.2em] uppercase text-white/30">{stat.label}</div>
              </motion.div>
            ))}
          </div>
        </motion.div>
